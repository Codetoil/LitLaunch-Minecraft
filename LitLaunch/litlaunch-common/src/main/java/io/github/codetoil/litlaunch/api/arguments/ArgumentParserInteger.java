...
public class ArgumentParserInteger implements IArgumentParser<IArgumentValue<Integer>> {
    ...
    public ArgumentValue<Integer> parse(String input) {
        ...
        }  //else FrontEnd.DO_THING_OBJECT().notifyUser("[" + LaunchCommon.NAME + " v" + LaunchCommon.VERSION + "] Warning: Please precede all integers with a `#` sign to make sure it registers as an integer", IDoThing.Color.YELLOW);
